...
wb = load_workbook(excel_path)  # 获取已存在的工作簿
# 获取所有工作表的名称列表
ws = wb.active
cnt = 0
for row in range(2,242):
    if ws.cell(row, 9).value is None or ws.cell(row, 9).value == '???':
        cnt+=1
no = 0
for row in range(2,242):
    if ws.cell(row, 9).value is None or ws.cell(row, 9).value == '???':
        ...
        print()
        try:
            bro.get(ws.cell(row, 11).value)
            sleep(random.randint(4, 10), ws.cell(row, 11).value)
            inter_text = bro.page_source
            tree2 = etree.HTML(inter_text)
            ws.cell(row, 9).value = str(tree2.xpath("//div[@class='job-sec-text']/text()"))
            # print('|'.join(job_miaoshu))
            ws.cell(row, 10).value = tree2.xpath("//div[@class='detail-section-item company-address']/div/div[1]/text()")[0]
...
#             try:
#                 bro.get(row[10])
#                 sleep(random.randint(5, 15), row[10])
#                 inter_text = bro.page_source
#                 tree2 = etree.HTML(inter_text)
