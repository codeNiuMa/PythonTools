...
for i in page:
    ...
    def op(li):
        ...
        print('大图网址：' + big_img_url)  # https://wallhaven.cc/w/rrowwm
        # 切分得到图片名字
        name = str(big_img_url).rsplit('/', 1)[-1]  # /wallhaven-rrowwm.png -> rrowwm.png
        if os.path.exists(fr'{path}\yuange666\{name}.jpg') is False and os.path.exists(
                fr'{path}\yuange666\{name}.png') is False:
            # 请求大图实际网址，并解析到图片源文件
