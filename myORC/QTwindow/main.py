...
class Stats:
    ...
    def showTime(self):
        ...
        timeDisplay = time.toString('yyyy/MM/dd hh:mm:ss')# 设置时间格式
        self.ui.label_time.setText(timeDisplay)        # 在Label上显示时间
    ...
    # 防止翻译进程卡死 增加多线程翻译
    def trans(self):
        thread = Thread(target=self.newTheadFun)
        thread.start()
    def newTheadFun(self):
